...
MAINTAINER zerchin
RUN sed -i "s/dl-cdn.alpinelinux.org/mirrors.aliyun.com/g" /etc/apk/repositories && apk update --no-cache && apk add --no-cache curl tcpdump tcptraceroute iperf3 fio bridge-utils bash jq openssh-client busybox-extras iftop bash-completion && cp /usr/share/zoneinfo/Asia/Shanghai /etc/localtime && echo "Asia/Shanghai" > /etc/timezone
